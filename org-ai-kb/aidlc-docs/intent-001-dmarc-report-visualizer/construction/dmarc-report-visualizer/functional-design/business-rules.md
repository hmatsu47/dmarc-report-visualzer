...
- 各レコードの`source_ip`に対して逆引きDNSを実行し、`reverse_dns`フィールドに格納する
- タイムアウト: 2秒（ソケットレベル）
- 逆引き失敗時（タイムアウト、NXDOMAIN等）: `None`（Parquetではnull）
- 処理に失敗してもレコード処理は続行する

## BR-12: header_fromの抽出

- XMLの`record/identifiers/header_from`要素からFromヘッダーのドメインを抽出する
- `identifiers`要素または`header_from`子要素が存在しない場合: `None`
